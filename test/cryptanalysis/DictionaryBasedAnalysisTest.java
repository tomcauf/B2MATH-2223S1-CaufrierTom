...
public class DictionaryBasedAnalysisTest {
	...
	void guessApproximatedAlphabetTest() {
		...
		for (int i = 0; i < DECODING_ALPHABET.length(); i++) {
			if (DECODING_ALPHABET.charAt(i) == alphabet.charAt(i))
				score++;
	...
	void getCompatibleWord() {
		...
		motCompatible = DictionaryBasedAnalysis.getCompatibleWord(motChiffre, dict.getWordsOfLength(motChiffre.length()));
	...
	void guessAlphabetWithStartAlphabetWithSpecialCharacter(){
		DictionaryBasedAnalysis dba = new DictionaryBasedAnalysis(cryptogramText, dictionary);
		assertThrows(IllegalArgumentException.class, () -> dba.guessApproximatedAlphabet("ABCDEFGHIJKLMNSTUVWXYZéàAB"));
	}
